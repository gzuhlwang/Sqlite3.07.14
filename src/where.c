...
*/
/*
**VDBE(虚拟数据库引擎),VDBE语言的每条指令都包括一个操作码和三个操作数,P1，P2，P3.
**P1是一个任意整数,P2是一个非负数,P3是一个结构体指针（可能为空）或者非空的字符串.
**很少的指令使用全部三个操作数.大多数指令仅仅使用一个或者两个操作数.也有一些指令
**没有操作数,而是操作在执行栈上的数据.每条指令的操作的细节和操作数的含义在操作符
**描述文档中.
*/
/*

**where．C负责处理SQL语句中WHERE子句的代码的生成.
**该模块用产生的一个循环的代码通过查询一个表来找适用的行.
**当选择使用索引加快查找速度,是适用的.
**因为这个模块负责选择索引,你也可以认为这个模块是“查询优化”.
*/
#include "sqliteInt.h"
